...
import { doc, getDoc, addDoc, collection, updateDoc, where, query } from "firebase/firestore";
    ...
    methods: {
        ...
        updateBlog() {
            if (this.blogTitle.length !== 0 && this.blogHTML.length !== 0) {
                const getPost = doc(db, "blogPosts", this.routeID);
                if (this.file) {
                    this.loading = true
                    ...
                    uploadTask.on("state_changed",
                        ...
                        (error) => {
                            ...
                            await getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
                                ...
                                updateDoc(dbRef, user_data).then(async (docRef) => {
